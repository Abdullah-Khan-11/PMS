...
router.get('/', checkLoginUer, function (req, res, next){
   ...
   res.render('add-new-category', { title : 'Password Management System', loginUser : loginUser, success : '', msg : '', errors :''});
  });
  
router.post('/', checkLoginUer, [ check('passwordCategory', 'Enter Password Category').isLength({ min : 1})], function (req, res, next){
    var loginUser = localStorage.getItem('loginUser');
    const errors = validationResult(req);
    if (!errors.isEmpty()){
      res.render('add-new-category', { title : 'Password Management System', loginUser : loginUser, success : '', msg : '', errors : errors.mapped()});
    }else{
    ...
    passCatDetails.save((err, doc)=>{
      if (err) throw err;
      res.render('add-new-category', { title : 'Password Management System', loginUser : loginUser, success : 'Password Category Inserted Successfully', msg : '', errors :''});
    }); 
   }
  });
